...
* Fixed exported NSFe to start at song #2.
* Fixed crash when MIDI notes are out of range.
* Fixed rasterization issues in the pattern thumbnails
...
* The register viewer will now show the real not pitches and wont necessarely match the notes on the piano roll
* Vibrato depths of more than 13 can unfortunately be supported in the sound engine at the moment. A warning will be given instead of producing the wrong result.
* Releases for VRC7/EPSM instrument will now correctly handle releases. That was always the intention, but a bug was preventing them from working
