            ...
            foreach (var regWrite in regWrites){
                switch (regWrite.Register){
                    ...
                    case NesApu.EPSM_ADDR0:
                        if (regWrite.Value >= 0x2D && regWrite.Value <= 0x2F){  //Prescaler setting
                            firstPass.Add(regWrite);
                    ...
                    case NesApu.EPSM_DATA0:
                        if (EPSMA0Addr < NesApu.EPSM_REG_SHAPE && regWrite.Value != EPSMSSGStatus[EPSMA0Addr]){ //Aka EPSM SSG regs that are saved
                            ...
                            firstPass.Add(regWrite);
                        } else if ((EPSMA0Addr & 0xF4) == 0xA4){    //hi pitch/block / CH3 hi pitch/block
                            EPSMFMHiPitchBuffer = regWrite;
                            EPSMFMHiPitchBuffered = true;          
                        } else if ((EPSMA0Addr & 0xF4) == 0xA0 && EPSMFMHiPitchBuffered){ //lo pitch / CH3 lo pitch
                    ...
                    case NesApu.EPSM_DATA1:
                        if ((EPSMA1Addr & 0xF4) == 0xA4){    //hi pitch/block / CH3 hi pitch/block
                            EPSMFMHiPitchBuffer = regWrite;
                            EPSMFMHiPitchBuffered = true;          
                        } else if ((EPSMA1Addr & 0xF4) == 0xA0 && EPSMFMHiPitchBuffered){ //lo pitch / CH3 lo pitch
                            EPSMFMHiPitchBuffered = false;
            ...
            for (int i = 0; i < firstPass.Count; i++){
                ...
                switch (regWrite.Register){
                    case NesApu.EPSM_ADDR0:
                        ...
                        (regWrite.Value >= 0x2D && regWrite.Value <= 0x2F)){ // Or selected a prescaler    
                            result.Add(regWrite);
                            EPSMA0Addr = regWrite.Value;
                        }
                        break;
                    case NesApu.EPSM_ADDR1:
                        ...
                        && regWrite.Value != EPSMA1Addr){  // If it actually wrote to EPSM 
                            result.Add(regWrite);
                            EPSMA1Addr = regWrite.Value;
                        }
                        break;
        ...
        public static RegisterWrite[] RemoveExpansionWritesBut (RegisterWrite[] writes, ushort chipMask){
            ...
            foreach (var w in writes){
                if (
                    ...
                    (w.Register == NesApu.N163_SILENCE && (useN163 || useS5B || useVRC7))   //Frickin' bus conflicts
                ) output.Add(w);
            }
