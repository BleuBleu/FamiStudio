...
{
    public partial class Direct2DResourceFontLoader : CallbackBase, FontCollectionLoader, FontFileLoader
    {
        ...
        public Direct2DResourceFontLoader(Factory factory)
        {
            _factory = factory;
            foreach (var name in typeof(Direct2DResourceFontLoader).Assembly.GetManifestResourceNames())
            {
                if (name.EndsWith(".ttf"))
                {
                    var fontBytes = Utilities.ReadStream(typeof(Direct2DResourceFontLoader).Assembly.GetManifestResourceStream(name));
