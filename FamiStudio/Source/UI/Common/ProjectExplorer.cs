...
namespace FamiStudio
{
    public class ProjectExplorer : RenderControl
    {
        ...
        RenderBitmapAtlas bmpAlgorithmAtlas;
        ...
        readonly string[] AlgorithmImageNames = new string[]
        {
            "Algorithm0",
            "Algorithm1",
            "Algorithm2",
                ...
                if (instrument != null && instrument == expandedInstrument)
                {
                    ...
                    if (instrumentParams != null)
                    {
                        foreach (var param in instrumentParams)
                        {
                            ...
                            {
                                buttons.Add(new Button(this) { type = GetButtonTypeForParam(param), param = param, instrument = instrument, color = instrument.Color, text = param.Name, textBrush = ThemeResources.BlackBrush, paramScope = TransactionScope.Instrument, paramObjectId = instrument.Id, atlas = bmpAlgorithmAtlas, atlasIdx = param.GetValue()});
                            }
                            else
        ...
        protected override void OnRenderInitialized(RenderGraphics g)
        {
            ...
            bmpAlgorithmAtlas = g.CreateBitmapAtlasFromResources(AlgorithmImageNames);
        ...
        protected override void OnRenderTerminated()
        {
            ...
            Utils.DisposeAndNullify(ref bmpAlgorithmAtlas);
        }
            ...
            for (int i = 0; i < buttons.Count; i++)
            {
                ...
                if (y + buttonSizeY >= 0)
                {
                        ...
                        button.type == ButtonType.ParamList)
                    {
                        if (firstParam)
                        {
                            for (int j = i + 1; j < buttons.Count; j++, numButtonsInGroup++)
                            {
        ...
        private void UpdateImage(Button sliderDragButton)
        {
            if (sliderDragButton.param.Name == "Algorithm")
                sliderDragButton.atlasIdx = sliderDragButton.param.GetValue();
        }
